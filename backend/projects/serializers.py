...
class ProjectListSerializer(serializers.ModelSerializer):
    professors = ProfessorNameSerializer(many=True, read_only=True)

    class Meta:
        model = Project
        fields = ["id", "project_name", "open_slots", "meeting_schedule", "professors"]
        extra_kwargs = {
    ...
    def validate(self, data):
        ...
        if open_slots is not None and capacity is not None:
            if open_slots > capacity:
                raise serializers.ValidationError(
                    "open slots should not exceed capacity"
                )
        return data
...
class ProfessorDashboardProjectSerializer(serializers.ModelSerializer):
    professors = ProfessorNameSerializer(many=True, read_only=True)
